 ...
 * dest[out]: Pointer to the output buffer holding each source bit's RMT counterpart (that means there are 8 additions to
 *           this buffer for a single src uint8_t element)
 * src_size: The number of uint8_t src elements to iterate through
 ...
 *           This number should exactly equal item_num / 8, since there are 8 rmt_items produced for a single src element
 * item_num[out]: The number of rmt_items that were appended to the dest buffer. This should exactly equal
 *           translated_size * 8, since there are 8 rmt_items per single src element
 */
static void IRAM_ATTR ws2812_rmt_translator(const void *src, rmt_item32_t *dest, size_t src_size,
